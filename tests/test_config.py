...
OPTIONAL_BLANK = {
    ...
    "DOWNLOAD_WATCH_INTERVAL_SECONDS": "",
    "DOWNLOAD_QUIET_SECONDS": "",
    "DOWNLOAD_IMPORT_MODE": "",
...
def test_unset_optionals_use_defaults(clean_env):
    ...
    assert settings.download_import_mode == "copy"
...
def test_blank_optionals_use_defaults(clean_env, monkeypatch):
    ...
    assert settings.sync_interval_minutes == 30
    assert settings.download_watch_interval_seconds == 30
...
def test_values_still_win(clean_env, monkeypatch):
    ...
    monkeypatch.setenv("DOWNLOAD_IMPORT_MODE", "move")
    ...
    assert settings.download_import_mode == "move"
