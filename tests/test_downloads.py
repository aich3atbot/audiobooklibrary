...
@respx.mock
def test_grab_release_adds_magnet_and_records_hash(clean_db, book):
    mock_details()
    deluge = mock_deluge()

    release = grab_release(clean_db, book, GUID, "AudioBookBay", "Noobtown [M4B]", 1000)
...
def test_grab_release_survives_a_torrent_deluge_already_has(clean_db, book):
    ...
    release = grab_release(clean_db, book, GUID, "AudioBookBay", "Noobtown", None)
...
def test_grab_failure_leaves_state_untouched(clean_db, book):
    ...
    with pytest.raises(Exception):
        grab_release(clean_db, book, GUID, "AudioBookBay", "Noobtown", None)
...
def test_grab_route_closes_modal_and_updates_card(client, clean_db, book):
    ...
    assert "grabbed" in response.text  # out-of-band card swap shows the new badge
